...
## Q8. コンパクション戦略 (v1.3.0 改訂)

長ミッションは prompt-too-long に当たります。**エージェントは `/compact` をプログラム的に呼べません** — どのハーネス(Claude Code / Codex / Cursor / Gemini)でもユーザー入力スラッシュコマンド。スキルは *提案* するだけ。実行はあなた(または `/schedule` cron)。

- **threshold** — トークン使用率がこの割合を超えたら助言を出す。
  - いつ? トークン重い作業(大 diff、大ファイル)は低め(60-70)、通常 docs/コードサイクルは高め(80-90)。デフォルト **80**。
- **strategy**:
  - `auto-suggest` *(デフォルト)* — 閾値超過で `📦 /compact 推奨` を 1 行出力。あなたが `/compact` を入力。
  - `cron-promote` — 本当に長いなら `/schedule "<cron> /autopilot"` を推奨。cron で起こるサイクルは毎回フレッシュ・セッションなのでコンパクション不要。
  - `off` — 沈黙。短い bounded ミッション専用。
